...
st.set_page_config(page_title="Sotto Scorta")
...
directory = r'Data/VariazioniSottoScorta'
#directory = r'C:\\Users\\vincenzo.citignola\\Documents\\GitHub\\reportSI\\Data\\VariazioniSottoScorta'
...
st.header("SOTTO SCORTA", divider='orange')
...
df = df[df["STOCK IN GUARDAROBA"]>0]
df_count_articoli_sottoscorta = df.groupby(by="DATA", as_index=False)["STOCK IN GUARDAROBA"].mean()

'''
Andamento medio di articoli in sottoscorta per ogni giorno. 
Il sovrascorta sono tutti gli STOCK < 7
'''

st.line_chart(df_count_articoli_sottoscorta, x="DATA", y="STOCK IN GUARDAROBA")
...
df1 = df1[df1["STOCK IN GUARDAROBA"]>0]
...
df_sottoscorta = df1.loc[df1["DATA"]==df1["DATA"].max()]


df_sottoscorta2 = df_sottoscorta.groupby(by="DESCRIZIONE GUARDAROBA", as_index=False)["STOCK IN GUARDAROBA"].count() # Numero di articoli in sovrascorta

'''
In questa sezione è il riportato il numero di articoli in Sottoscorta per ogni Guardaroba alla data di ultimo aggiornamento.
Si intende sottoscorta quando lo stock (tenuto conto del consegnato medio e della giacenza presente) è minore di 7
'''

#df_sovrascorta2 = df_sovrascorta2.loc[df_sovrascorta2["STOCK IN GUARDAROBA"]>7]
df_sottoscorta2 = df_sottoscorta2.sort_values(by="STOCK IN GUARDAROBA", ascending=False)
st.dataframe(df_sottoscorta2, hide_index=True, column_order=("DESCRIZIONE GUARDAROBA", "STOCK IN GUARDAROBA"),
                                                            column_config={"CARICATO IN GUARDAROBA ULTIMI 7 GIORNI": None,
                                                                           "SPEDITO DA LAVANDERIA ULTIMI 7 GIORNI": None,
                                                                           "STOCK IN GUARDAROBA": st.column_config.TextColumn("ARTICOLI IN SOTTOSCORTA")
                                                                           })
...
st.dataframe(df_medio2, hide_index=True,column_config={"STOCK IN GUARDAROBA": st.column_config.TextColumn("NUM. ARTICOLI IN SOTTOSCORTA IN MEDIA"),"CARICATO IN GUARDAROBA ULTIMI 7 GIORNI": None,
                                       "SPEDITO DA LAVANDERIA ULTIMI 7 GIORNI": None})
...
df_sottoscorta_today= df_guardaroba2.loc[df_guardaroba2["DATA"]==df_guardaroba2["DATA"].max()]


col1, col2 = st.columns(2)
col1.metric(label=":orange[Stock odierno]", value=int(df_sottoscorta_today["STOCK IN GUARDAROBA"]))
col2.metric(label=":orange[Stock medio]", value=int(df_guardaroba2["STOCK IN GUARDAROBA"].mean()))
